...
final class ArtWorkListCellView: UITableViewCell {
    ...
    func setupView() {
        ...
        NSLayoutConstraint.activate([
            ...
            artworkImageView.bottomAnchor.constraint(lessThanOrEqualTo: bottomAnchor, constant: -12),
            ...
            artworkDateLabelView.bottomAnchor.constraint(lessThanOrEqualTo: bottomAnchor, constant: -12)
        ])
    }
    
    func configure(for model: ViewModel) {
        artworkImageView.kf.setImage(with: URL(string: model.imageURL))
        artworkTitleLabelView.text = model.title
        artworkDateLabelView.text = model.date
        artworkArtistLabelView.text = model.artistName
    }
}
